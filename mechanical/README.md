...
当前使用 OpenSCAD 做参数化 Rev 0.3。先打印公差件，再打印定位板和底壳；不要在没有装入实际轴体、两个 EC11、摇杆和 RGB 灯前锁定外壳尺寸。
...
- 底壳高度：18 mm。
- 固定方式：M3 螺丝 + 热熔铜螺母。
- 前侧 quota RGB 灯：12 个独立窗口，间距 12.5 mm。

这些数值是 Rev 0.3 的可打印起点，不是最终量产尺寸。
...
按 `F6` 渲染后导出 STL。当前 Rev 0.3 的 STL 已提交到 `mechanical/exports/`，方便直接下载和导入切片器。
...
脚本生成的模型与当前 Rev 0.3 的 OpenSCAD 参数保持一致；修改尺寸时需要同步检查两个源文件。
