# Rev 0.9.4 从 0 到 1 手焊装配说明书

这份说明书按“**没有定制 PCB，直接把元件焊到导线上**”的路线编写，适合第一次焊接键盘的人。现在全部硬件已经到齐，按下面的总图从上层面板、主控板、RGB 灯带到最终合盖依次完成。

## 先看装配视频

[播放/下载：Rev 0.9.4 封闭盒体装配视频](media/assembly_walkthrough.mp4)
...
1. ① 把上层面板翻到背面，焊 13 个按键、13 个二极管和 EC11。
2. ② 把 Arduino Micro 卡入下层封闭盒体，再接 D4–D11、D2、D3、D12 和 GND。
3. ③ 最后接 12 颗 RGB：A3→330Ω→DIN，5V/GND 并联。
4. ④ 开盖通电测试，全部正常后再合上上下层面板。

**第一次通电前必须用万用表确认 5V 和 GND 没有短路。**

## 按键面板背面细节

![面板背面实物焊接示意](diagrams/panel_backside_soldering.svg)

把面板翻到背面，看到黑色按键和两个银色针脚；黄色小元件是二极管。每个按键左边接二极管，二极管黑环朝橙色行线，按键右边接蓝色列线。
...
5. 用万用表检查，再把 8 根矩阵线接到 Arduino。

### B 键的具体焊点

如果你现在正在焊 B 键，只看下面这张图：

![B 键三个焊点的实物位置示意](diagrams/b_key_welding_closeup.svg)

B 键有三个相关焊点，但**不是三个东西焊在同一个焊点上**：
...
![BCDE 共用 D5 行线](diagrams/bcde_row_bus_closeup.svg)

是的，B、C、D、E 属于同一行，所以把**四个二极管的黑环端**用一根橙色线连起来，这根线最后标记为 `D5`。不要把 B、C、D、E 的按键针脚直接互相连接。
...
- 3D 打印的 `plate.stl`、`bottom.stl`、`pixel_carrier.stl` 和封闭用 `window_lens.stl`
...
![封闭盒体与主板卡槽装配关系](diagrams/closed_box_assembly.svg)
...
6. 安装并焊接立式 EC11。
7. Arduino Micro 先用临时线连接并上传固件，再永久整理线束。
8. 最后接 RGB 灯带、电阻和电容。
9. 安装透明窗片，合上上层控制盖，完成最终测试。
...
6. 将 35×18×10 mm Arduino Micro 放入 `bottom.stl` 的卡槽：先让后端贴住后挡块，再向下压入左右导轨；确认 USB-C 插座落在前墙开口中心。
7. 不要把导线压进卡槽；主板两侧与导轨约有 0.6 mm 间隙，焊点和元件不能顶住导轨。
...
不要把 EC11 的公共脚直接接 5V。固件使用 Arduino 的 `INPUT_PULLUP`，按上表接线。

## 第 7 步：接 RGB 灯带

接灯带前先确认它是 5V 可寻址型号，然后：

1. 将 12 颗灯放入 `pixel_carrier.stl`，确认正面朝向面板窗口。
2. 先只接第 0 颗灯、330Ω 电阻、5V 和 GND。
3. 单颗测试通过后，再串接其余 11 颗。
...
![最终封闭盒体装配关系](diagrams/closed_box_assembly.svg)

1. 拔掉 USB-C。
2. Arduino Micro 卡入 `bottom.stl` 的 35×18×10 mm 主控卡槽，USB-C 对准下层盒体前墙开口。
3. 将 RGB 灯和导线固定在 `pixel_carrier.stl`，确保灯珠正面朝向面板窗口。
4. 整理矩阵和 EC11 线束，所有裸露焊点加热缩管或绝缘胶带。
5. 从下方将 `window_lens.stl` 或同尺寸透明亚克力片压到主板观察窗边缘；这一步是封闭主板的必要步骤。
6. 用四个 M3×16mm 螺丝锁入下层盒体四角固定柱，先松装，确认线束没有被夹住。
...
- [ ] 主控板到货后先在未合盖状态测试，再进行最终装配。
- [ ] 主控板完全落入 35×18×10 mm 卡槽，Type-C 插口由盒体前墙承受插拔力。
