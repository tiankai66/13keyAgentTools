...
[        4 × 3 按键矩阵        ]
[上层控制盖：按键、旋钮、水平 RGB 和透明主板窗]
[下层封闭盒体：35×18×10 mm Arduino Micro 卡槽 + 前墙 Type-C 开口]
```

音量旋钮使用 D2/D3/D12。面板上的 12 颗 RGB 灯使用 A3 串联，按灯珠顺序从左到右显示剩余用量；12 颗灯共用一个数据引脚，通过 DIN/DOUT 串联，不占用 12 个 GPIO。

Arduino Micro 平放在下层封闭盒体的卡槽内，左右导轨和后挡块负责限位，USB-C 由前墙 14×9 mm 开口引出。建议先将板卡和 USB 线固定，再从下方安装透明窗片，最后把上层控制盖通过四角 M3 螺丝锁紧。
