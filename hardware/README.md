...
- Arduino Micro 竖向固定在右侧红框空白区下方的独立下层面板 18 × 48 mm 承托架上，USB-C 朝图示前侧上边缘；上层面板由四角 10 mm 支柱支撑。
- 所有模块共用 GND。
...
详细引脚见 [wiring/pinout.md](wiring/pinout.md)，采购清单见 [purchase-list.md](purchase-list.md)，结构化物料表见 [bom.csv](bom.csv)。本版本是上下两块独立打印面板，不再使用封闭式 24 mm 底壳；图示前侧 USB-C 开口和主板透明观察窗已同步建模。
