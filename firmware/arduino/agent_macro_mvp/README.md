...
- 音量旋钮：F11/F12 临时占位，后续改为 USB Consumer volume up/down

以上快捷键是 MVP 占位符；不同 Agent 终端的实际快捷键应在主机端确认后再固化，固件本身保持通用 HID 宏键盘定位。

## RGB

默认 `ENABLE_RGB` 为 `0`，这样可以先不安装 RGB 库和 RGB 灯测试按键/旋钮输入。
...
`QUOTA 0`～`QUOTA 100` 会把面板 12 颗独立 RGB 灯显示为剩余用量条：0% 全灭，100% 全亮；中间像素按百分比部分亮起，颜色在红、黄、绿之间渐变。首版亮度限制为 32/255，避免 USB 供电过载。
